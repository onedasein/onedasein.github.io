...
#note[
  一句话：*AI Infra = 系统功底 ∩ 机器学习*。门槛高、供给少、溢价高，本科生能拿出真作品的人极少——这是三个方向里「差异化收益」最大的一个，也是*最不能靠看视频学会*的一个。
]
...
#tbl(
  (1.7fr, 1fr),
  [任务], [产出],
  [官方 #link("https://docs.nvidia.com/cuda/cuda-c-programming-guide/")[CUDA C Programming Guide] 通读 + 示例跑通], [笔记],
...
*核心概念（必须能自己讲清）*：warp/warp divergence、shared memory bank conflict、coalesced access、occupancy 与寄存器压力、`__syncthreads`、异步拷贝（cp.async）、warp shuffle。
...
- #link("https://triton-lang.org/")[*Triton*]：写 fused attention / layernorm / matmul，对比手写 CUDA 的开发效率。
- 读 #link("https://github.com/Dao-AILab/flash-attention")[FlashAttention] 论文 + 源码，理解 tiling 与 online softmax。
...
*资源*：#link("https://github.com/NVIDIA/Megatron-LM")[Megatron-LM]、#link("https://github.com/microsoft/DeepSpeed")[DeepSpeed]、#link("https://pytorch.org/docs/stable/distributed.html")[PyTorch Distributed 文档]、论文《ZeRO》《Megatron-LM》《FlashAttention》。
...
#tbl(
  (auto, 1.8fr),
  [内容], [关键点],
  [#link("https://docs.vllm.ai/")[vLLM]], [PagedAttention、continuous batching、前缀缓存（prefix caching）],
...
- 课：#link("https://www.cs.cmu.edu/~418/")[CMU 15-418] / UIUC ECE408（并行体系结构）｜#link("https://docs.nvidia.com/cuda/cuda-c-programming-guide/")[CUDA C Programming Guide]
...
#note[
  ① 手写 FlashAttention 级 kernel + ncu 报告；② 2 卡 FSDP 吞吐优化报告；③ vLLM 推理服务调优报告 + 自研最小 continuous batching 调度器；④ 一篇拆解论文的 writeup。这套组合在本科实习生里属于前 1%。
]
